...
#if defined(NDEBUG)
# error "tongbaocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc88d669a9b56a31fbea6d691abe7e1c00cb78f11dff62e0bf358fce74a172d4c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // tongbaocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "tongbaocoin Signed Message:\n";
...
{
    // tongbaocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is is reduced by 50%  every 4 years
    nSubsidy >>= (nHeight / 900000); // tongbaocoin: 900k blocks in ~4 years
 if(nHeight == 1)
     {
       nSubsidy = 90000000 * COIN;	// 60 million coins, that all coins for not mine
       return nSubsidy + nFees;
     }   
 else if(nHeight < 210210)
	{
		nSubsidy = 0 * COIN;	
		return nSubsidy + nFees;
	}
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // tongbaocoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // tongbaocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // tongbaocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // tongbaocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "tongbaocoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1453042799;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 545198;

        if (fTestNet)
        {
            block.nTime    = 1453035600;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa539eff0ba63030429e37d20ba63e735fdad9a6db8f973249e127bea46fdf6ca"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("tongbaocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("tongbaocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static tongbaocoinMiner(CWallet *pwallet)
{
    printf("tongbaocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tongbaocoin-miner");
    ...
    try { loop {
        ...
        printf("Running tongbaocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("tongbaocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&tongbaocoinMiner, pwallet));
}
